...
public class DrivetrainSubsystem extends SubsystemBase {
  ...
  public DrivetrainSubsystem() {
    ...
    swerveOdo = new SwerveDriveOdometry(m_kinematics, getGyroscopeRotation(), new Pose2d(0, 0, new Rotation2d()));
   ...
   addDashboardEntries(tab.getLayout("Odo", BuiltInLayouts.kList)
        .withSize(2, 4)
        .withPosition(0, 0), swerveOdo.getPoseMeters());
    ...
    //
    // Mk4iSwerveModuleHelper.createNeoNeo(...)
    ...
    // By default we will use Falcon 500s in Mk4i configuration. But if you use a different configuration or motors
  ...
  public void periodic() {
   // System.out.println(swerveOdoPt2.getPoseMeters());

    SwerveModuleState[] states = m_kinematics.toSwerveModuleStates(m_chassisSpeeds);

    if(Math.abs(m_chassisSpeeds.omegaRadiansPerSecond)<.001 
       && Math.abs(m_chassisSpeeds.vxMetersPerSecond)<.001
       && Math.abs(m_chassisSpeeds.vyMetersPerSecond)<.001)
    {
          m_frontLeftModule.set(0, m_frontLeftModule.getState().angle.getRadians());
          m_frontRightModule.set(0, m_frontRightModule.getState().angle.getRadians());
          m_backRightModule.set(0, m_backRightModule.getState().angle.getRadians());
          m_backLeftModule.set(0, m_backLeftModule.getState().angle.getRadians());
          return;
    }
    ...
    swerveOdo.update(getGyroscopeRotation(), m_frontLeftModule.getState(), m_frontRightModule.getState(), m_backLeftModule.getState(), m_backRightModule.getState());

    var angle = getGyroscopeRotation();
    ...
    //Pose2d m_pose = swerveOdo.update(getGyroscopeRotation(), states);
